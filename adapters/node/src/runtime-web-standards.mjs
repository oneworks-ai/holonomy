...
export class RuntimeURL {
  constructor(value, base) {
    ...
    if (/^[a-z][a-z\d+.-]*:/iu.test(input)) this.#assign(parseAbsolute(input))
    else {
      ...
      const directory = parent.pathname.endsWith('/')
        ? parent.pathname
      ...
      this.#assign({
        ...
        pathname: normalizePath(path.startsWith('/') ? path : directory + path),
        port: parent.port,
...
export class RuntimeTextDecoder {
  ...
  decode(input = new Uint8Array()) {
    ...
    for (let index = 0; index < bytes.length;) {
      ...
      if (
        (width === 2 && point < 0x80) || (width === 3 && point < 0x800) ||
        (width === 4 && point < 0x10000) || point > 0x10FFFF || (point >= 0xD800 && point <= 0xDFFF)
      ) {
