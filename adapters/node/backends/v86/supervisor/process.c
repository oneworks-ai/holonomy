...
static pid_t launch_child(const struct holo_spawn_request *request, int pipes[3][2]) {
    pid_t pid = fork();
    if (pid != 0) return pid;
    if (setpgid(0, 0) != 0 || chdir(request->cwd) != 0) _exit(126);
...
int holo_process_spawn(struct holo_process_table *table, int control_fd, const struct holo_frame *frame) {
    ...
    pid = launch_child(&request, pipes);
    if (pid < 0) {
        close_pipes(pipes);
        holo_spawn_request_free(&request);
