...
static int read_stream(int control_fd, struct holo_process *process, enum holo_poll_stream stream) {
    ...
    if (count == 0 || (count < 0 && errno != EAGAIN && errno != EINTR)) close_fd(fd);
...
int holo_process_poll_event(int control_fd, const struct pollfd *fd, const struct holo_poll_target *target) {
    ...
    return read_stream(control_fd, target->process, target->stream);
}
