...
describe('runtime composer V4 contracts', () => {
  ...
  it('accepts the mock-only Network capability without an HTTP capability', async () => {
    const input = options(['host.network.mock']) as HolonomyRuntimeOptions & { network: unknown }
    input.network = { authority: { allowedOrigins: ['https://example.test'] }, principal: 'principal' }
    const runtime = await createHolonomyRuntime(input)
    expect(runtime.network).toBeDefined()
    expect(runtime.globals.fetch).toBeTypeOf('function')
    await runtime.dispose()
  })
