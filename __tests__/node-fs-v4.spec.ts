...
describe('node-fs V4 native bridge conformance', () => {
  it('enforces virtual root traversal and stable domain errors', async () => {
    ...
    await expect(settle(loop, fs.promises.readFile('mobile-fs://workspace/missing'))).rejects.toMatchObject({
      code: 'ENOENT'
    })
    await expect(fs.promises.mkdir('mobile-fs://workspace/%2e%2e/escape')).rejects.toMatchObject({ code: 'EINVAL' })
  ...
  it('uses Bridge-issued opaque resources for FileHandle calls and closes exactly once', async () => {
    ...
    const handle = await settle(loop, fs.promises.open('mobile-fs://workspace/data.bin', 'w+'))
  ...
  it('streams large reads through V4 credit/backpressure and permits cancellation', async () => {
    const { fs, loop, port } = setupMemoryFs()
    await settle(loop, fs.promises.writeFile('mobile-fs://workspace/large.txt', 'abcdefgh'))
    const stream = fs.createReadStream('mobile-fs://workspace/large.txt')
    ...
    const cancelled = fs.createReadStream('mobile-fs://workspace/large.txt', { signal: controller.signal })
  ...
  it('resolves symlinks only inside an authority and honors O_NOFOLLOW', async () => {
    const { fs, loop } = setupMemoryFs()
    await settle(loop, fs.promises.writeFile('mobile-fs://workspace/target.txt', 'ok'))
    await settle(loop, fs.promises.symlink('mobile-fs://workspace/target.txt', 'mobile-fs://workspace/link.txt'))
    await expect(settle(loop, fs.promises.open('mobile-fs://workspace/link.txt', 0x20000))).rejects.toMatchObject({
      code: 'EPERM'
    })
    const linkStat = await settle(loop, fs.promises.lstat('mobile-fs://workspace/link.txt'))
    expect(linkStat.isSymbolicLink()).toBe(true)
    await expect(settle(loop, fs.promises.symlink('mobile-fs://app-data/target', 'mobile-fs://workspace/x'))).rejects
      .toMatchObject({ code: 'EXDEV' })
  })

  it('delivers watch events only after a stream credit is granted', async () => {
    const { fs, loop } = setupMemoryFs()
    await settle(loop, fs.promises.writeFile('mobile-fs://workspace/watched.txt', 'before'))
    const watcher = fs.watch('mobile-fs://workspace/')
    const event = watcher.next()
    await settle(loop, fs.promises.writeFile('mobile-fs://workspace/watched.txt', 'after'))
  ...
  it('rechecks root permissions, quotas and cross-authority boundaries', async () => {
    ...
    const first = await settle(limited.loop, limited.fs.promises.open('mobile-fs://workspace/one.txt', 'w'))
    await expect(settle(limited.loop, limited.fs.promises.open('mobile-fs://workspace/two.txt', 'w'))).rejects
    ...
    await expect(settle(
      ...
      limited.fs.promises.rename(
        'mobile-fs://workspace/one.txt',
        'mobile-fs://app-data/one.txt'
      )
    )).rejects.toMatchObject({ code: 'EXDEV' })

    const readOnly = setupMemoryFs({ permissions: ['metadata', 'read'] })
    const denied = settle(readOnly.loop, readOnly.fs.promises.writeFile('mobile-fs://workspace/nope.txt', 'x'))
  ...
  it('supports O_DIRECTORY, follows only intermediate nofollow links, and snapshots guest options', async () => {
    const { fs, loop } = setupMemoryFs()
    await settle(loop, fs.promises.mkdir('mobile-fs://workspace/directory'))
    const directory = await settle(loop, fs.promises.open('mobile-fs://workspace/directory', fs.constants.O_DIRECTORY))
    const directoryStat = await settle(loop, directory.stat())
    expect(directoryStat.isDirectory()).toBe(true)
    await directory.close()
    await settle(loop, fs.promises.writeFile('mobile-fs://workspace/file.txt', 'x'))
    await expect(settle(loop, fs.promises.open('mobile-fs://workspace/file.txt', fs.constants.O_DIRECTORY))).rejects
      .toMatchObject({ code: 'ENOTDIR' })
    await expect(settle(
      loop,
      fs.promises.open(
        'mobile-fs://workspace/no-ghost',
        fs.constants.O_CREAT | fs.constants.O_DIRECTORY
      )
    )).rejects.toMatchObject({ code: 'EINVAL' })
    await expect(settle(loop, fs.promises.stat('mobile-fs://workspace/no-ghost'))).rejects
      .toMatchObject({ code: 'ENOENT' })

    const hostile = {}
    Object.defineProperty(hostile, 'timeoutMs', { enumerable: true, get: () => 1 })
    await expect(fs.promises.stat('mobile-fs://workspace/file.txt', hostile as never)).rejects
      .toMatchObject({ code: 'EINVAL' })
    await expect(fs.promises.stat('mobile-fs://workspace/file.txt', { unknown: true } as never)).rejects
      .toMatchObject({ code: 'EINVAL' })
  })
  ...
  it('keeps prior contents when an opaque atomic write cannot complete', async () => {
    ...
    await settle(loop, fs.promises.writeFile('mobile-fs://workspace/atomic.txt', 'old'))
    await expect(settle(loop, fs.promises.writeFile('mobile-fs://workspace/atomic.txt', 'toolong'))).rejects
      .toMatchObject({ code: 'ENOSPC' })
    await expect(settle(loop, fs.promises.readFile('mobile-fs://workspace/atomic.txt', { encoding: 'utf8' })))
      .resolves.toBe('old')
    ...
    await expect(settle(
      loop,
      fs.promises.writeFile('mobile-fs://workspace/never-created.txt', 'x', { signal: controller.signal })
  ...
  it('closes rogue unary and stream resource grants before surfacing EIO', async () => {
    ...
    await expect(settle(loop, fs.promises.stat('mobile-fs://workspace/a'))).rejects.toMatchObject({ code: 'EIO' })
    expect(closed).toBe(1)
    expect(bridge.getSnapshot().openResources).toBe(0)
    expect(cancelled).toBe(0)

    const stream = fs.createReadStream('mobile-fs://workspace/a')
  ...
  it('maps V4 deadlines to a redacted ETIMEDOUT error and disposes streams', async () => {
    ...
    await settle(loop, fs.promises.writeFile('mobile-fs://workspace/timeout.txt', 'pending'))
    const watcher = fs.watch('mobile-fs://workspace/timeout.txt', { timeoutMs: 10 })
    ...
    const second = fs.createReadStream('mobile-fs://workspace/timeout.txt')
