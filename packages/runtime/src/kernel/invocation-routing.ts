import { normalizeNetworkInvocationSnapshotV1 } from '@holonomyjs/capability-network/kernel/network-invocation'
import { buildNetworkInvocationSnapshotV1 } from '@holonomyjs/capability-network/kernel/network-invocation-builder'
import {
  canonicalizeProcessNetworkEndpointResource,
  canonicalizeProgramExecutableResource,
  canonicalizeShellExecutableResource
} from '@holonomyjs/capability-process/kernel/canonical-process-resources'
import { routeDescendantProcessInvocationV1 } from '@holonomyjs/capability-process/kernel/process-descendant-routing'
import {
...
export const routeCapabilityInvocationV1 = (input: {
  ...
  if (input.module === 'holo:runtime' && input.member === 'authorizeDescendantProcess') {
    const args = input.arguments
    if (args == null || typeof args !== 'object' || Array.isArray(args)) {
      throw new CapabilityInvocationError('argument.invalid', 'process.program.spawn')
    }
    return routeDescendantProcessInvocationV1(args as Readonly<Record<string, JsonValueV1>>)
  }
