...
export class HolonomyRuntimePluginAppV1 {
  ...
  constructor(options: RuntimePluginAppOptionsV1) {
    ...
    this.#createContextService = options.createContextService
  ...
  async replace(input: unknown): Promise<RuntimePluginGraphSnapshotV1> {
    ...
    try {
      this.#publish(nextRevision, definitions)
    } catch (error) {
      await Promise.allSettled([...staged.values()].reverse().map(item => item.dispose()))
      throw error
    }
  ...
  async #install(definition: RuntimePluginDefinitionV1, key: string): Promise<RuntimePluginInstanceV1> {
    ...
    const lifecycle = Object.freeze({
      ...
      registerDispose: (dispose: () => void | Promise<void>) => {
        if (typeof dispose !== 'function') throw new TypeError('Runtime plugin disposer is invalid')
        disposers.push(dispose)
      }
    })
    const service = this.#createContextService?.(definition, lifecycle) ?? Object.freeze({})
    const context = this.#context.extend({
      holo: Object.freeze({ instanceId: definition.instanceId, ...service })
    })
    let fiber
    try {
      fiber = context.plugin(plugin as never, definition.config as never)
      await fiber
    } catch (error) {
