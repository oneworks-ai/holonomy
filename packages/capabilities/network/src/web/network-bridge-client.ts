...
export class NetworkBridgeClient {
  ...
  private readonly pendingCancels = new Map<string, (reason?: string) => void>()
  ...
  async request(
    ...
    try {
      ...
      const result = await this.awaitRequest(request, options)
  ...
  dispose() {
    ...
    this.disposed = true
    this.generation += 1
    for (const cancel of [...this.pendingCancels.values()]) cancel('network_runtime_disposed')
  ...
  private awaitRequest(
    ...
    return new Promise((resolve, reject) => {
      ...
      const cancel = (reason = 'abort') => {
        if (!finish()) return
        this.bridge.cancel(request.id, reason)
        reject(createWebNetworkError('network.cancelled'))
      }
      this.pendingCancels.set(request.id, cancel)
      try {
        removeAbort = this.listenForAbort(options.signal, () => cancel('abort'))
        if (options.signal?.aborted) {
          cancel('abort')
          return
        }
        void this.bridge.request(request, this.callOptions(options)).then(
          result => {
            if (!finish()) {
              closeResultResources(result, 'late_network_result')
              return
            }
            resolve(result)
          },
