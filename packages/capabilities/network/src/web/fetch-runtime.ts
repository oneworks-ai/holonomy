...
import type { NativeResourceHandle, NativeStream } from '@holonomyjs/runtime/native-port/types'
...
const MAX_RESPONSE_READERS = 128
...
interface NativeResponseReaderState {
  cursor: number
}

class NativeResponseBodyBranch implements WebBodySource {
  constructor(
    private readonly source: SharedNativeResponseBody,
    private readonly readerId: number
  ) {}

  cancel(reason?: string) {
    this.source.cancelReader(this.readerId, reason)
  }

  clone() {
    return this.source.cloneReader(this.readerId)
  }

  pull() {
    return this.source.pullReader(this.readerId)
  }
}

class SharedNativeResponseBody {
  ...
  private readonly readers = new Map<number, NativeResponseReaderState>()
  ...
  createReader() {
    return this.addReader(this.baseCursor)
  }

  cloneReader(readerId: number) {
    const reader = this.readers.get(readerId)
    if (reader == null) throw createWebNetworkError('network.cancelled')
    return this.addReader(reader.cursor)
  }

  async pullReader(readerId: number): Promise<Uint8Array | undefined> {
    for (;;) {
      const reader = this.readers.get(readerId)
      if (reader == null) throw createWebNetworkError('network.cancelled')
      const index = reader.cursor - this.baseCursor
  ...
  cancelReader(readerId: number, reason?: string) {
    if (!this.readers.delete(readerId)) return
    this.pruneChunks()
    if (this.readers.size === 0 && !this.done && this.failure == null) this.cancel(reason)
  }
  ...
  private addReader(cursor: number) {
    if (this.readers.size >= MAX_RESPONSE_READERS) throw createWebNetworkError('network.protocol_error')
    const readerId = this.nextReaderId++
    this.readers.set(readerId, { cursor })
    return new NativeResponseBodyBranch(this, readerId)
  }

  private async fill() {
    this.filling ??= this.readNext().finally(() => {
  ...
  private async readNext() {
    ...
    try {
      ...
      if (!this.isPullCurrent(cancellationEpoch)) {
        ...
        this.failure = createWebNetworkError('network.cancelled')
        return
      }
      this.close('read_error')
      this.onFailed((error as { code?: string })?.code ?? 'network.internal', false)
      this.failure = this.client.normalizeError(error)
      return
    }
    if (!this.isPullCurrent(cancellationEpoch)) {
      this.close('late_response_body')
    ...
    if (
      ...
      this.fail('network.protocol_error', 'protocol_error')
      return
    }
    this.totalBytes += binary[0].data.byteLength
    if (this.totalBytes > this.authority.limits.maxResponseBodyBytes) {
      this.fail('network.response_too_large', 'response_too_large')
      return
    }
    if (!this.isPullCurrent(cancellationEpoch)) {
      this.close('late_response_body')
  ...
  private fail(
    code: 'network.protocol_error' | 'network.response_too_large',
    reason: string
  ) {
    if (this.closed) return
    this.failure = createWebNetworkError(code)
    this.cancellationEpoch += 1
    this.stream?.close(reason)
    void this.client.request(WEB_NETWORK_OPERATIONS.http.cancel, {
      response: this.resource
    }).catch(() => undefined)
    this.onFailed(code, false)
    this.close(reason)
  }
  ...
  private pruneChunks() {
    ...
    const minimum = Math.min(...[...this.readers.values()].map(reader => reader.cursor))
    const count = minimum - this.baseCursor
    if (count <= 0) return
    this.chunks.splice(0, count)
    this.baseCursor = minimum
  }
}
...
class FetchRuntimeController {
  ...
  async fetch(input: WebRequestInfo, init: WebFetchInit = {}) {
    ...
    let ruleSnapshotId: string | undefined
    ...
    try {
      for (let count = 0;; count += 1) {
        ...
        const response = await this.issue(
          url,
          method,
          headers,
          body,
        ...
        ruleSnapshotId = response.ruleSnapshotId
        ...
        if (!REDIRECT_STATUSES.has(response.head.status) || location == null || redirect === 'manual') {
          try {
            await this.releaseRuleSnapshot(ruleSnapshotId)
            ruleSnapshotId = undefined
          } catch (error) {
            this.closeConnection(response.lease, 'rule_snapshot_release_failed')
            throw error
          }
      ...
      if (ruleSnapshotId != null) {
        await this.releaseRuleSnapshot(ruleSnapshotId).catch(() => {})
      }
  ...
  private async issue(
    ...
    existingRuleSnapshotId?: string
  ) {
    const lease = this.acquireConnection()
    let createdRuleSnapshotId: string | undefined
    try {
      const started = await this.client.request(WEB_NETWORK_OPERATIONS.http.request, {
        capabilityBindingId: capabilityAdmission?.bindingId ?? null,
        headers: [...headers].map(([name, value]) => [name, value]),
        method,
        ...(existingRuleSnapshotId == null ? {} : { ruleSnapshotId: existingRuleSnapshotId }),
        url: url.toString()
      }, options)
      const resource = getResponseResource(started)
      lease.resource = resource
      this.assertCurrent(generation, lease)
      const accepted = readRecord(decodeNetworkValue(started.value))
      if (accepted.accepted !== true) throw createWebNetworkError('network.protocol_error')
      const returnedRuleSnapshotId = accepted.ruleSnapshotId
      if (
        returnedRuleSnapshotId != null &&
        (typeof returnedRuleSnapshotId !== 'string' || !/^network-rules:\d{1,20}$/u.test(returnedRuleSnapshotId))
      ) throw createWebNetworkError('network.protocol_error')
      if (existingRuleSnapshotId != null && returnedRuleSnapshotId !== existingRuleSnapshotId) {
        throw createWebNetworkError('network.protocol_error')
      }
      if (existingRuleSnapshotId == null && typeof returnedRuleSnapshotId === 'string') {
        createdRuleSnapshotId = returnedRuleSnapshotId
      }
      ...
      return {
        ...
        options,
        resource,
        ruleSnapshotId: returnedRuleSnapshotId as string | undefined
      }
    } catch (error) {
      this.closeConnection(lease, 'request_failed')
      if (createdRuleSnapshotId != null) {
        await this.releaseRuleSnapshot(createdRuleSnapshotId).catch(() => {})
      }
      throw error
    }
  }

  private async releaseRuleSnapshot(ruleSnapshotId: string | undefined) {
    if (ruleSnapshotId == null) return
    const result = await this.client.request(WEB_NETWORK_OPERATIONS.http.releaseRules, { ruleSnapshotId })
    if (readRecord(decodeNetworkValue(result.value)).released !== true) {
      throw createWebNetworkError('network.protocol_error')
    }
  }
  ...
  private async toResponse(
    ...
    const sharedBody = response.head.hasBody
      ? new SharedNativeResponseBody(
        this.client,
        response.resource,
        response.options,
        this.authority,
      ...
      : undefined
    response.lease.body = sharedBody
    ...
    const releaseCapability = capabilityAdmission == null || this.options.capability == null
      ? undefined
      : () => this.options.capability!.releaseResponse(capabilityAdmission)
    return WebResponse.fromNetwork({
      body: new WebBodyController(sharedBody?.createReader(), releaseCapability),
