...
static int write_init(int fd, const struct fuse_in_header *input, ssize_t count) {
    struct {
        struct fuse_out_header header;
        struct fuse_init_out body;
    } output;
    const struct fuse_init_in *request;
    if (count < (ssize_t)(sizeof(*input) + sizeof(*request))) return write_error(fd, input->unique, EPROTO);
    request = (const struct fuse_init_in *)((const uint8_t *)input + sizeof(*input));
    if (request->major != 7U) return write_error(fd, input->unique, EPROTO);
    memset(&output, 0, sizeof(output));
    output.header = (struct fuse_out_header){ sizeof(output), 0, input->unique };
    output.body.major = 7U;
    output.body.minor = request->minor < 39U ? request->minor : 39U;
    output.body.flags = FUSE_ATOMIC_O_TRUNC;
    output.body.max_background = 16U;
    output.body.congestion_threshold = 12U;
    output.body.max_write = 64U * 1024U;
    output.body.time_gran = 1U;
    output.body.max_pages = 16U;
    return write(fd, &output, sizeof(output)) == (ssize_t)sizeof(output) ? 0 : -1;
}
...
int holo_fuse_bridge_init(struct holo_fuse_bridge *bridge) {
    ...
    snprintf(
        options,
        sizeof(options),
        "fd=%d,rootmode=40000,user_id=1000,group_id=1000,max_read=65536",
        bridge->fd
    );
    if (mount("holo-fs", "/workspace", "fuse", MS_NODEV | MS_NOEXEC | MS_NOSUID, options) != 0) {
        close(bridge->fd);
        bridge->fd = -1;
        return -1;
    }
...
int holo_fuse_bridge_poll(struct holo_fuse_bridge *bridge, struct holo_process_table *processes,
    ...
    if (input->opcode == FUSE_INIT) {
        int result = write_init(bridge->fd, input, count);
        free(bytes);
        return result;
    }
