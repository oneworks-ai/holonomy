...
- The only exported Android compatibility entry accepts a random bounded `commandId` and forwards it to the non-exported foreground Supervisor. Full command-v2 envelopes, module graphs, argv/env, initial controls and Inspector options remain app-private. Test discovery, test wrappers and TAP/JSON reporting stay in JavaScript; the machine-level Service owns device selection, ADB transfer, host fixture lifecycle, reverse/forward leases and CDP proxying. Kotlin does not branch on test files, understand suites/cases, parse Network Mock text or implement OpenAPI.
...
- External source modules retain the absolute hierarchical scheme returned by the injected host resolver and stay under that configured root. `holonomy:` is reserved for manifest-verified internal assets; `node:` resolves only through the host synthetic registry.
...
- Planning and engine evaluation are separate. The platform loader accepts a host-defined absolute hierarchical root scheme and owns package planning; Android/Javet executes canonical sources supplied by `RuntimeModuleResolver` and does not reinterpret or rename their URL identity.
