...
const fsConstraints = (input: MaterializationInputV1, available: boolean) => {
  ...
  if (input.descriptor.operation === 'filesystem.entry.rename') {
    ...
    let shared = 0
    while (destination.pathSegments[shared] === resource.pathSegments[shared]) shared += 1
...
const deviceConstraints = (input: MaterializationInputV1) => {
  ...
  return { maxPrecision, maxPrivacyTier: ceiling.maxPrivacyTier, operations: [resource.operation] }
}
