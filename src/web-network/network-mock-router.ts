...
export class NetworkMockRouter implements NativePort {
  ...
  private async downstreamUnary(
    ...
    const request: NativePortRequest = {
      args: operation === WEB_NETWORK_OPERATIONS.http.request
        ? { headers: exchange.headers, method: exchange.method, url: exchange.url }
        : { response: reference! },
      ...(binary == null ? {} : { binary }),
  ...
  private openRequest(
    ...
    if (
      args == null || Reflect.ownKeys(args).length !== 3 || request.binary != null || context.resources.length !== 0 ||
