...
| 检查点               | 当前状态               | 目标                                                                                                                                                                                                                                                              | 退出证据                                                                                                                                                                                            |
| -------------------- | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| M3.5-U：HoloUV 基础  | 已完成 v1              | 共享 Environment Runtime、Backend Driver、Host OS Adapter 与 Guest System Adapter；JS 保持 Node-compatible facade，Backend 对齐 libuv 的 handle/request/loop/process/stream/FS/network 语义；v86 daemon 为 `/sbin/holo-uvd`，wire protocol 不暴露 `uv_*` 内存结构 | Native Darwin 与 v86 共用 operation/resource/lifecycle vectors；`holo-uvd` 的 spawn/stdio/signal/close/restart E2E；平台差异只出现在 Adapter                                                        |
...
| M3.5-B：v86 能力桥接 | 已完成 Experimental v1 | 后代 exec 准入与 Host-only gate deadline；绝对 `execve`、受限 `execveat`、PATH 绝对解析和 executable TOCTOU fail-closed；Linux FS、TCP/UDP/DNS 与 Host Device/System 投影通过版本化 Host↔Guest 通道进入同一 Broker，并保留 Linux PID/PPID/executable attribution  | Node/Desktop 与 Android emulator 的文件、网络、设备、后代 allow/deny、generation restart E2E；Host 逐字段控制 Device/System；相对 dirfd、`AT_EMPTY_PATH`、未知/可变目标稳定拒绝且不回退 ambient API |
...
| Linux 文件、TCP/UDP/DNS、Host Device/System 注入与后代执行准入 | M3.5-B       | 已完成 Experimental v1 | 全部重新进入同一 Broker，保留 environment、PID/PPID、executable attribution，并覆盖 TOCTOU/fencing |
...
- HoloUV 是 M3.5-U 的公共执行抽象，不是只服务 `spawn()` 的临时 Supervisor。它统一环境生命周期、handle/request、stdio、process tree 与异步终态；Darwin、Linux、Windows 等差异由 Host OS Adapter 与 Guest System Adapter 承担。
...
- Linux 内的 `curl`、`git` 和任意程序发起的连接归 M3.5-B 的 Process Network Bridge，必须携带 environment、PID/PPID 与 executable attribution；JS `fetch()` 仍归 M3-N，两者共享 Network Policy owner，但不是同一个 operation。
